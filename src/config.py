...
REFERENCE_TRANSLATIONS_PATH = DATASET_DIR / get_env_or_default(
    "REFERENCE_FILE", "reference_translations.txt"
)

# Model settings (configurable via environment variables)
SENTIMENT_MODEL = get_env_or_default(
    "SENTIMENT_MODEL", "distilbert-base-uncased-finetuned-sst-2-english"
)
TRANSLATION_MODEL = get_env_or_default(
    "TRANSLATION_MODEL", "Helsinki-NLP/opus-mt-en-es"
)
SUMMARIZATION_MODEL = get_env_or_default(
    "SUMMARIZATION_MODEL", "cnicu/t5-small-booksum"
)
QA_MODEL = get_env_or_default("QA_MODEL", "deepset/minilm-uncased-squad2")
TOPIC_MODEL = get_env_or_default(
    "TOPIC_MODEL", "all-MiniLM-L6-v2"
)  # Updated for BERTopic
ASPECT_SENTIMENT_MODEL = get_env_or_default(
    "ASPECT_MODEL", "yangheng/deberta-v3-base-absa-v1.1"
)
...
LOG_FORMAT = get_env_or_default(
    "LOG_FORMAT", "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
LOG_FILE = (
    PROJECT_ROOT / "logs" / get_env_or_default("LOG_FILE", "car_reviews_analysis.log")
)
...
def get_all_configs() -> Dict[str, Any]:
    ...
    return {
        ...
        "export": {"formats": EXPORT_FORMATS, "save_plots": SAVE_PLOTS},
    }
