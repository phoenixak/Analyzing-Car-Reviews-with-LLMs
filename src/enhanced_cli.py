...
class EnhancedCLI:
    ...
    def display_results_summary(self, results: Dict[str, Any]):
        ...
        for task_name, task_data in results.items():
            if isinstance(task_data, dict):
                ...
                summary_table.add_row(task_name.title(), status, metrics or "N/A", output or "N/A")
    ...
    def prompt_for_confirmation(self, message: str) -> bool:
        ...
        return click.confirm(click.style(f"🤔 {message}", fg="yellow", bold=True))
    ...
    def display_help_examples(self):
        ...
        examples = [
            ...
            (
                ...
                "python main.py --task all --save-results --verbose",
            ),
            ("Topic modeling only", "python main.py --task topic --visualize"),
            (
                "Custom dataset",
                "python main.py --data-file /path/to/data.csv --task sentiment",
            ),
            (
                "Multiple specific tasks",
                "python main.py --task sentiment,translation,qa --visualize",
            ),
        ]
    ...
    def display_model_info(self):
        ...
        for task, model_config in config["models"].items():
            ...
            models_table.add_row(task.title(), model_name, task_type, status)
...
@click.option("--verbose", is_flag=True, help="Enable verbose output with detailed progress")
@click.option("--show-config", is_flag=True, help="Display current configuration and system info")
@click.option("--show-examples", is_flag=True, help="Display usage examples and exit")
@click.option("--show-models", is_flag=True, help="Display available models and their status")
@click.option(
    "--preload-models",
    is_flag=True,
...
@click.option(
    "--evaluate",
    is_flag=True,
    help="Run sentiment model evaluation on a built-in test set and print metrics",
)
def enhanced_main(
    data_file,
    task,
    ...
    if evaluate:
        ...
        enhanced_cli.console.print("[bold]Running sentiment model evaluation...[/bold]")
        try:
            metrics = evaluate_sentiment_model()
            # Display results table
            results_table = Table(title="Sentiment Evaluation Results")
            results_table.add_column("Metric", style="cyan")
            results_table.add_column("Value", style="green")

            results_table.add_row("Model", metrics["model_name"])
            results_table.add_row("Samples", str(metrics["num_samples"]))
            ...
            results_table.add_row("F1 Score", f"{metrics['f1']:.4f}")

            enhanced_cli.console.print(results_table)

            f1 = metrics["f1"]
            if f1 >= 0.80:
                enhanced_cli.console.print(f"[bold green]PASS[/bold green] -- F1 {f1:.2%} >= 80%")
            else:
                enhanced_cli.console.print(f"[bold red]FAIL[/bold red] -- F1 {f1:.2%} < 80%")
        except Exception as e:
            enhanced_cli.console.print(f"[red]Evaluation failed: {e}[/red]")
    ...
    try:
        ...
        # Build arguments list to pass directly
        args_list = ["--data-file", data_file, "--task", task]

        if visualize:
            args_list.append("--visualize")
        if save_results:
            args_list.append("--save-results")
        if verbose:
        ...
        with enhanced_cli.create_progress_tracker("Analysis Pipeline") as progress:
            ...
            main_task = progress.add_task(
                "Running analysis...", total=len(tasks) if tasks != ["all"] else 7
            )

            # Execute analysis, passing args directly
            original_main(argv=args_list)
