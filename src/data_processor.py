...
class EnhancedDataProcessor:
    ...
    def _load_json(self, file_path: Path, **kwargs) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                labels = [item.get('label', item.get('class', item.get('sentiment', ''))) for item in data]
    ...
    def _extract_reviews_and_labels(self, df: pd.DataFrame, file_path: Path) -> Tuple[List[str], List[str]]:
    ...
    def _validate_data(self, reviews: List[str], labels: List[str], file_path: Path) -> Tuple[List[str], List[str]]:
    ...
    def preprocess_reviews(self, reviews: List[str], 
                          min_length: int = 10, 
                          max_length: int = 5000,
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            ...
            for task_name, task_data in results.items():
                if isinstance(task_data, dict) and "predictions" in task_data:
                    ...
                    predictions_df.to_excel(writer, sheet_name=task_name[:31], index=False)  # Excel sheet name limit
